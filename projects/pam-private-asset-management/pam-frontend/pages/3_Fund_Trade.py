...
def load_fund_trades():
	"""Load fund trades from API"""
	return api.get_data("/api/v1/trades/fund")

@st.cache_data(ttl=30)
def load_fund_portfolios():
	"""Load fund portfolios to get current holdings"""
	try:
		# Lấy tất cả assets
		...
		# Lấy fund portfolios
		portfolios = api.get_data(f"/api/v1/assets/{fund_asset['id']}/fund-portfolios/")
		return portfolios if portfolios else []
	except Exception:
		return []

def calculate_fund_holdings():
	"""Calculate current fund holdings from trades"""
	trades = load_fund_trades()
	if not trades:
		return {}
...
# --- 1. FUND LIST (Current Holdings) ---
st.header("📊 Current Fund Holdings")
...
holdings = calculate_fund_holdings()
...
if holdings:
	...
	for ticker in holdings.keys():
		if st.session_state.get(f"show_sell_{ticker}", False):
			...
			def sell_fund_dialog(ticker_symbol=ticker):
				...
				st.info(f"**Current Holdings:** {current_qty:,.2f} CCQ | **Avg Price:** {avg_price:,.0f} VND")
				
				with st.form(f"sell_form_{ticker_symbol}", clear_on_submit=True, border=False):
					col1, col2 = st.columns(2)
					with col1:
						sell_quantity = st.number_input(
							"Quantity to Sell (CCQ)", 
							min_value=0.01, 
					...
					with col2:
						sell_total_value = st.number_input(
							"Total Value ('000 VND)", 
							min_value=0.0, 
							step=0.01,
							format="%.2f",
							help="E.g., 1525.50 = 1,525,500 VND",
							key=f"total_value_{ticker_symbol}"
						)
					...
					with col4:
						sell_tax = st.number_input("Tax (VND)", min_value=0, step=100, format="%d", key=f"tax_{ticker_symbol}")
					
					# Calculate total amount received
					if sell_quantity > 0 and sell_total_value > 0:
						total_value = sell_total_value * 1000  # Convert to actual VND
						price_per_unit = total_value / sell_quantity if sell_quantity > 0 else 0
						total_received = total_value - sell_fee - sell_tax
						
						st.write(f"**Price per Unit:** {price_per_unit:,.0f} VND")
						st.write(f"**Total Received:** {total_received:,.0f} VND")
					...
					with col_submit:
						if st.form_submit_button("✅ Confirm Sell", type="primary"):
							if sell_quantity > 0 and sell_total_value > 0:
								# Calculate price per unit from total value
								actual_price = (sell_total_value * 1000) / sell_quantity
								trade_data = {
									"trade_type": "sell",
								...
								with st.spinner("Recording sell trade..."):
									...
									if result:
										st.success(f"✅ Successfully sold {sell_quantity} units of {ticker_symbol}!")
										st.cache_data.clear()
										# Reset session state
			...
			sell_fund_dialog(ticker)
else:
	st.info("No fund holdings found. Start by buying some funds!")

# --- 2. NEW TRADE FORM ---
st.divider()
st.header("➕ Record New Fund Trade")
...
# Pagination controls
if 'fund_page' not in st.session_state:
	st.session_state.fund_page = 1

trades_data = load_fund_trades()
...
if trades_data is not None:
	if trades_data:
		...
		# Apply pagination
		start_idx = (st.session_state.fund_page - 1) * items_per_page
		...
		with col1:
			if st.button("⏮️ First", disabled=st.session_state.fund_page == 1):
				st.session_state.fund_page = 1
				st.rerun()
		with col2:
			if st.button("⬅️ Prev", disabled=st.session_state.fund_page == 1):
				st.session_state.fund_page -= 1
				st.rerun()
		with col3:
			st.write(f"Page {st.session_state.fund_page} of {total_pages}")
		with col4:
			if st.button("Next ➡️", disabled=st.session_state.fund_page == total_pages):
				st.session_state.fund_page += 1
				st.rerun()
		with col5:
			if st.button("Last ⏭️", disabled=st.session_state.fund_page == total_pages):
				st.session_state.fund_page = total_pages
				st.rerun()
		...
		st.info("No fund trades recorded yet.")
else:
	st.error("Failed to load trade data. Please try again.")

# --- Footer ---
st.markdown("---")
st.markdown("*Fund Trading Log - Last updated: " + datetime.now().strftime("%d/%m/%Y %H:%M") + "*")