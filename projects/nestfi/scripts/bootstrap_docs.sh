...
mkdir -p docs/product docs/architecture docs/business docs/qa docs/delivery \
         ...
         .pane_heartbeats .pane_watches

# v10.18 — Pre-create PRODUCT_IDEA.md with a placeholder template so the
# Orchestrator never hits a "file does not exist" path on first paste of
# the user's idea (Claude's Write tool was finicky about that case).
if [[ ! -f PRODUCT_IDEA.md ]]; then
  cat > PRODUCT_IDEA.md <<'EOF'
# Product Idea

> Paste the user's project idea here (or write it directly). The framework
> reads this file as the canonical source of intent before Discovery starts.
> All workers (PM, BA, UX, SA) read this file as their first input.

(awaiting user input — replace this placeholder)
EOF
  echo "Created PRODUCT_IDEA.md placeholder"
fi
