You are running in the BE tmux pane for this product engineering project.
...
fake the missing context. Follow the guidance printed by the script:
  ...
  • If a USER-owned file (e.g. PRODUCT_IDEA.md) is missing, run:
        bash scripts/ask_orchestrator.sh BE "<question stating what is missing>"

  • If an agent-owned file is missing, run:
        bash scripts/notify_orchestrator.sh BE "Cannot proceed — missing inputs: <comma-separated list>. Need <upstream owners> to produce them first. Please coordinate and re-route me when ready."

Then append a "blocked on missing inputs" entry to memory/BE.md
...
- memory/BE.md            — what you (BE) decided previously
...
memory/_PROJECT_STATE.md and memory/BE.md already carry the
cross-role context you'd otherwise re-derive from those skipped files.

# STEP 3 — do the work

Task from caller:
Phase 3 (IMPLEMENTATION_PLANNING): Backend planning task. Read: docs/architecture/TECH_STACK.md, SOLUTION_ARCHITECTURE.md, ADR.md, API_CONTRACT.md. Read: docs/product/PRD.md, docs/business/USER_STORIES.md. Write: planning/BE_PLAN.md with (1) module breakdown (auth, users, families, transactions, categories, admin), (2) ORM/migration strategy (SQLAlchemy models, Alembic versioning), (3) API endpoint priorities (core endpoints v phase 2), (4) dependencies and risks. If you need clarification on tech stack choices, call ask_orchestrator.sh. Notify when ready for coding phase.
