...
**Hard enforcement (v10.19 + v10.23)** — TWO layers:

1. **PATH guard (shell-level, v10.19)**: `scripts/guards/` is
   pre-pended to your pane's `PATH` by `start_agents_tmux.sh`. If you
   shell-invoke `docker`/`psql`/`python`/`npm`/etc., the wrapper
   fires and prints `BLOCKED — Orchestrator cannot execute engineering
   commands`. Do NOT bypass with `command <cmd>` or absolute paths.
2. **PreToolUse hook (tool-level, v10.23)**: a hook on `Write`/`Edit`
   tools runs `scripts/guards/check_file_lane.sh`. If you try to edit
   `backend/*`, `frontend/*`, `docs/product/PRD.md`, etc., the hook
   exits 2 and Claude reports `BLOCKED — Orchestrator cannot Write/Edit
   owner-specific files`. This closes the v10.19 loophole where the
   PATH guard couldn't see Claude's native file tools.

When you see either block, read the "Route to X" hint and call:

```bash
bash scripts/route_to_pane.sh X "<task>"
```

Files Orches IS allowed to Edit directly (everything else routes):
