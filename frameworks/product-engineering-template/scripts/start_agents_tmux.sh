...
FREE_MODE=false            # v10.16 — --free forces free-tier model overlay
...
while [[ $# -gt 0 ]]; do
  case "$1" in
    ...
    --free)
      FREE_MODE=true; shift ;;
    -h|--help)
      echo "Usage: bash scripts/start_agents_tmux.sh <project_name> [--engine opencode|claude] [--free]"
      echo
      echo "  --engine   Pick the CLI backend (default: opencode)."
      echo "  --free     Override every role's model with a free-tier ID"
      echo "             (sources config/engines/<engine>-free.env after the"
      echo "             main engine config). Use this for iterating on the"
      echo "             workflow without burning paid credits."
      exit 0 ;;
    *)
...
# v10.16 — --free overlay. Sourced AFTER the main engine config so it
# can override every *_MODEL variable. The overlay file is optional;
# if missing we warn and continue with the paid defaults.
if [[ "$FREE_MODE" == "true" ]]; then
  FREE_CFG="config/engines/${ENGINE}-free.env"
  if [[ -f "$FREE_CFG" ]]; then
    # shellcheck disable=SC1090
    source "$FREE_CFG"
    echo "[v10.16] --free mode active — sourced $FREE_CFG"
    echo "[v10.16] All 9 roles will use free-tier models for this session."
  else
    echo "[v10.16] WARN: --free passed but $FREE_CFG not found. Using paid defaults."
  fi
fi
...
# v10.18 — Bind prefix+W to wake_orchestrator. Useful when Claude's
# auto-wake fails (it types "." + Enter into the Orchestrator pane,
# which forces the mandatory inbox check at start-of-turn).
tmux bind-key -T prefix W run-shell "cd '$PROJECT_DIR' && bash scripts/wake_orchestrator.sh"
echo "[v10.18] tmux binding installed: Prefix+W → wake Orchestrator"
...
for AGENT in "${AGENTS[@]}"; do
  ...
  tmux send-keys -t "$PANE_ID" "export FREE_MODE='$FREE_MODE'" C-m
