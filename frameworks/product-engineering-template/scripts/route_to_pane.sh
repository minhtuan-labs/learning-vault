...
# v10.16 — honour FREE_MODE persisted in .agent_session. Sourced AFTER
# the main engine config so it overrides *_MODEL.
...
{
  echo "#!/bin/bash"
  # NOTE: no `set -e` because the worker command may exit non-zero from
  # model timeouts/aborts — we still want the safety-net auto-notify to run.
  echo ""
  echo "ROLE='${TARGET}'"
  echo ""
  echo "# v10.18 — Heartbeat: background loop touches the heartbeat file"
  echo "# every 10s so the watcher_daemon can detect a stalled worker."
  echo "mkdir -p .pane_heartbeats"
  echo "("
  echo "  while :; do"
  echo "    touch \".pane_heartbeats/\${ROLE}.beat\" 2>/dev/null || true"
  echo "    sleep 10"
  echo "  done"
  echo ") &"
  echo "HEARTBEAT_PID=\$!"
  echo "trap 'kill \$HEARTBEAT_PID 2>/dev/null; rm -f .pane_heartbeats/\${ROLE}.beat 2>/dev/null' EXIT"
  echo ""
  echo "# v10.18 — Capture start time so the auto-notify safety net can"
  echo "# tell the difference between 'just finished' and 'crashed instantly'."
  echo "START_TS=\$(date +%s)"
  echo ""
  echo "# Run the actual worker command."
  echo "$WORKER_CMD"
  echo "EXIT_CODE=\$?"
  echo ""
  echo "# v10.18 — Safety net: if the worker didn't call notify_orchestrator.sh"
  echo "# before exiting, fire one ourselves so the Orchestrator doesn't"
  echo "# stall waiting for a signal that's never coming. Dedup by checking"
  echo "# whether this role already filed a notify within the last 30s."
  echo "NEED_FALLBACK=true"
  echo "LATEST_NOTIF=\$(ls -t .pane_notifications/\${ROLE}_*.md 2>/dev/null | head -1 || true)"
  echo "if [[ -n \"\$LATEST_NOTIF\" ]]; then"
  echo "  MTIME=\$(stat -f %m \"\$LATEST_NOTIF\" 2>/dev/null || stat -c %Y \"\$LATEST_NOTIF\" 2>/dev/null || echo 0)"
  echo "  NOW=\$(date +%s)"
  echo "  AGE=\$((NOW - MTIME))"
  echo "  if (( AGE < 30 )); then"
  echo "    NEED_FALLBACK=false"
  echo "  fi"
  echo "fi"
  echo "if \$NEED_FALLBACK; then"
  echo "  bash scripts/notify_orchestrator.sh \"\$ROLE\" \"Engine session exited (code=\$EXIT_CODE) without an explicit completion notify. Check artifacts under role's output paths to verify status. (Auto-fired by framework safety net — worker may have legitimately finished without calling notify_orchestrator.sh, or may have crashed.)\" || true"
  echo "fi"
  echo ""
  echo "exit \$EXIT_CODE"
} > "$WORKER_SH"
