...
>
> - **v10.23 (Claude engine only) Write/Edit hook**: Claude Code's
>   `PreToolUse` hook is registered in `.claude/settings.json` to
>   block Orchestrator from editing owner-specific files via Claude's
>   native file tools. OpenCode is unaffected because the loophole
>   was specific to Claude Code's tool model. The PATH guard (v10.19)
>   covers both engines for shell commands.
>
